...
def compute_differences(orig, susp):
    ...
    if orig.shape != susp.shape:
        # naive resize if shapes differ: center-crop or pad is better in production
        # here we will resize susp to orig using numpy (simple scaling)
        import skimage.transform as tf
        susp = tf.resize(susp, orig.shape, anti_aliasing=True)

    mse = float(np.mean((orig - susp) ** 2))
    # structural_similarity expects data_range (max-min)
