...
const NoiseprintReport: React.FC<NoiseprintReportProps> = ({ imageFile }) => {
  ...
  // Usuwamy automatyczne generowanie po załadowaniu obrazu,
  // bo teraz robimy to przyciskiem
  useEffect(() => {
    if (!imageFile) {
      setNoiseprint(null);
      setStats(null);
      ...
      {!imageFile ? (
        ...
        <>
          <button
            ...
            disabled={loading}
            className="px-4 py-2 bg-teal-700 hover:bg-teal-600 disabled:bg-gray-600 text-white font-medium rounded-lg shadow transition"
          >
